...
typedef union _CSR_CTL_REG
{
    struct 
    {
        uint32_t    PrgDone:1; /*The processor sets this bit after completion of
                            Programming / Erase Operation / CMD43 / CMD20
                            Interrupt.*/
        uint32_t    WrLastBlkBusy:1;/*Write Last Block Busy Control
                                ...
                                program done bit.
                                */
        uint32_t    CardInitDone:1; /*On Program Start Interrupt, the processor will start
        ...
        uint32_t    AdrOutOfRange:1;/*This bit reflects in Card Status Register.
        ...
        uint32_t    AdrMisalign:1;/*This bit reflects in Card Status Register.
                            ...
                            WRITE_BL_LEN value.*/
        uint32_t    Rsvd1:1;
        uint32_t    EraseParam:1;/*This bit reflects in Card Status Register. .
                            1 - An invalid selection of erase groups for erase occurred.
                            0 – No Error*/
        uint32_t    CardEccFail:1;/*This bit reflects in Card Status Register.
                            1 - Card internal ECC was applied but failed to correct the data
                            0 – No Error*/
        uint32_t    CcErr:1;/*This bit reflects in Card Status Register.
                        1 - A card error occurred, which is not related to the host command.
                        0 – No Error*/
        uint32_t    Err:1; /*This bit reflects in Card Status Register.
                        A generic card error related to the (and detected
                        during) execution of the last host command (e.g.
                        read or write failures).
                        0 – No Error*/
        uint32_t    MmcIrqTrigger:1;/*This bit is used in MMC Interrupt Mode.
                                ...
                                states.*/
        uint32_t    CmdDataOutputEdge:1; /*Command and Data Output Edge:
                                    ...
                                    edge of SD/MMC/SDIO clock*/
        uint32_t    Cmd32Cmd33Ena:1;/*0 – Disabled. The SD/MMC/SDIO Device Controller
        ...
        uint32_t    BootSeqSupport:1;/*0 – Boot Sequence is not supported
                                    1 – Boot Sequence is supported*/
        uint32_t    BootPartEna:1;
        uint32_t    BootAck:1;
        uint32_t    WpViolation:1;
        uint32_t    WpEraseSkip:1;
        uint32_t    CidCsdOverwrite:1;
        uint32_t    AkeSeqErr:1;
        uint32_t    CardEccDisable:1;
        uint32_t    StreamThresholdSize:3;/*000 – 32Bytes
                                    001 – 64Bytes
                                    010 – 128Bytes
                                    011 – 256Bytes
                                    100 - 512Bytes
        ...
        uint32_t    PermanentWrProt:1;/*0 – The card is not permanently Write protected
                                    1 – The card is permanently write protected
                                    This field is required for internal lock unlock logic.*/
        uint32_t    TempWrProt:1; /*0 – The card is not Temporary Write protected
                                    1 – The card is Temporary write protected
                                    This field is required for internal lock unlock logic*/
        uint32_t    WpCmdEna:1;
        uint32_t    AllowAKE:1; /*This bit determines whether to allow AKE
                            ...
                            as illegal commands.*/
        uint32_t    SecuredMode:1;/*This bit determines whether to allow protected area
        ...
        uint32_t    AkeSeqOK:1; /*The Processor set this bit after AKE was successful
                            ...
                            next ACMD48.*/
        uint32_t    AssdDisEna:1; /*0 – ASSD Commands are Disabled.1 – ASSD Commands are Enabled*/
        uint32_t    BootDataRdy:1;/*0- Boot data is not ready from Firmware
...
typedef union _CSR_CMD_REG
{
    struct 
    {
        uint32_t    AppCmd:1;/*1 – Current command is an Application Command
                            0 – Not an Application Command.*/
        uint32_t    BlockSize:12;/*This field denotes the size of the data block.
                            12’d 0 – Reserved
                            12’d 1 – 1 Byte
                            12’d 2 - 2 Byte ....*/
        uint32_t    CmdIndex:6; /*This field denotes the Index of the Current command*/
        uint32_t    CurBusWidth:2; /*Denotes the current Bus Width
                                00 - 1 Bit
                                01 - 4 Bits
                                10 - 8 Bits
                                11 – Reserved*/
        uint32_t    CurSpeed:3; /*Defines the Speed Class Control Bits
                            ...
                            0100b: Update CI
                            Others: Reserved*/
        uint32_t    CardState:4; /*Defines the current state of the Controller.
                            0 = Idle
                            1 = Ready
                            2 = Ident
                            3 = Stby
                            ...
                            11–15 = reserved*/
        uint32_t    EraseSeq:1;/*Erase Sequence
...
typedef union _CSR_CMD52_ARG_REG
{
    struct 
    {
        uint32_t    WrData:8;
        uint32_t    Stuff_1:4;
        uint32_t    RegAdr:17;  // read/write start address in function's address space
        uint32_t    Stuff_2:1;
        uint32_t    RawFlag:1;  // read after write flag
        uint32_t    FuncNum:3;
        uint32_t    RWFlag:1;
    } bit;
    uint32_t    word;
...
typedef union _CSR_CMD53_ARG_REG
{
    struct 
    {
        uint32_t    ByteOrBlkCnt:9;  /* for byte mode, 0 means read/write 512 bytes. for block mode, 0 means infinite count.
...
the CCCR*/
        uint32_t    RegAdr:17;  // read/write start address in function's address space
        uint32_t    OpCode:1;  // fixed address or incrementing address
        uint32_t    BlkMode:1;
        uint32_t    FuncNum:3;
        uint32_t    RWFlag:1;
    } bit;
...
typedef union _CSR_DMA_CTL_REG
{
    struct 
    {
        uint32_t    DmaAdrVld:1; // DMA address valid
        uint32_t    BufSize:3;/*DMA1 Buffer Size
...
typedef union _CSR_PWD_LEN_REG
{
    struct 
    {
        uint32_t    PwdsLen:8; // password len
        uint32_t    DisableLockUnlock:1; // Lock/unlock feature is enable/disable
        uint32_t    Rsvd:23;
    } bit;
    uint32_t    word;
} CSR_PWD_LEN_REG;
...
typedef union _CSR_CCCR_CTL_REG
{
    struct 
    {
        uint32_t    CccrFmtVer:4;   /*CCCR Format Version Number.
        ...
        uint32_t    SdioSpecVer:4; /*SDIO Specification Revision Number
        ...
        uint32_t    SdxFmtVer:4; /*SD Format Version Number.
                                ...
                                04h-0Fh Reserved for Future Use
                                */
        uint32_t    S8B:1;//Support 8it bus mode.
        uint32_t    SCSI:1;/*Support Continuous Support of Interrupt, 0 – Continuous SPI Interrupt is supported, 
                       1 – Continuous SPI Interrupt is not supported.*/
        uint32_t    SDC:1;//Support Direct Command(CMD52)
        uint32_t    SMB:1;//Multiple Block Transfer
        uint32_t    SRW:1;//Support Read Wait
        uint32_t    SBS:1;//Support Bus Control,0 – Suspend/Resume is not supported, 1 – Suspend/Resume is supported
        uint32_t    S4MI:1; //Support Block Gap Interrupt
        uint32_t    LSC:1;  //Low Speed Card
        uint32_t    M4BLS:1;  // 4bit mode for low speed card
        uint32_t    SMPC:1; //Support for Master Power Control
        uint32_t    SHS:1;  //0 – High Speed is not supported  1 – High Speed is supported
        uint32_t    SDR50:1;
        uint32_t    SDR104:1;
        uint32_t    DDR50:1;
        uint32_t    SDTA:1;  //Driver Type A
        uint32_t    SDTC:1;  //Driver Type C
        uint32_t    SDTD:1; //Driver Type D
        uint32_t    SAI:1;  //Asynchronous Interrupt
        uint32_t    Rsvd:2;
    } bit;
    uint32_t    word;
} CSR_CCCR_CTL_REG;
...
typedef union _CSR_FBRX_CTL_REG
{
    struct 
    {
        uint32_t    StdFuncCode:4;  /*SDIO Standard Function Code.
                            ...
                            this case, the value in byte 101h identifies the standard SDIO interfaces type.
                                    */
        uint32_t    ExtStdFuncCode:8; /*Function X Extended Standard SDIO Function Interface Code.*/
        uint32_t    SupportCSA:1;  //code storage area
        uint32_t    SPS:1;
        uint32_t    Rsvd:15;
    } bit;
    uint32_t    word;
...
typedef union _CSR_CARD_OCR_REG
{
    struct 
    {
        uint32_t    CardOCR:24; /*Card OCR – Operational Condition Register.
                                ...
                                23 – 3.5 to 3.6.
                                        */
        uint32_t    SwitchTo1dot8V:1; /*1 - Switching to 1.8V accepted
                                    0 – Switching to 1.8V not accepted*/
        uint32_t    Rsvd1:4;
        uint32_t    AccessMode:1;/*Used only in MMC mode – tied to zero always
                                Based on bit30 only the modes are
                                differentiated (byte mode Vs sector mode)
                                Bit30 – 0 => byte mode
                                Bit30 – 1 => Sector mode
                                        */
        uint32_t    AccessModeCCS:1; /*MMC Mode – Access mode
...
typedef struct _SDIO_REG_
{
    ...
    uint32_t                    csr_blk_count;
    uint32_t                    csr_dma1_adr;
    CSR_DMA_CTL_REG         csr_dma1_ctl;
    uint32_t                    csr_dma2_adr;
    CSR_DMA_CTL_REG         csr_dma2_ctl;
    uint32_t                    csr_erase_wr_blk_start;
    uint32_t                    csr_erase_wr_blk_end;
    CSR_PWD_LEN_REG         csr_pwd_len;
    uint32_t                    csr_int_status;
    uint32_t                    csr_int_status_ena;
    uint32_t                    csr_int_signal_ena;
    CSR_CARD_ADR_REG        csr_card_adr;
    uint32_t                    csr_card_data;
    uint32_t                    csr_ioready;
